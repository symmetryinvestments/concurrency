...
// - Client or end-users might want to abort an earlier request they made.
...
// This module implementes cooperative cancellation using StopSources,
// StopTokens and StopCallbacks.
...
struct StopCallback {
	...
	/// 
	/// If the callback is still registered when a stop is requested the
	/// callback with be invoked exactly 1 time.
	///
	/// Note that the callback might be called during the execution of this
	/// function.
	///
	/// A StopCallback can only be registered with one StopToken.
	/// Registration with another first disposes of the old registration.

	/// TODO: what about when then callback is invoked before or while removing
	/// it from the old StopToken, do we want to register the new one?
	/// Or do we want to exit early returning some bool to indicate
	...
	/// if any.
	///
	/// A registered callback could still be triggered before this function
	/// returns, but it will have been ran to completion.
