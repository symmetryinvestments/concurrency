...
@("cleanup.voidsender.single") @safe
unittest {
	auto s = asyncScope();
...
@("cleanup.voidsender.triple") @safe
unittest {
	auto s = asyncScope();
	s.spawn(VoidSender()).should == true;
	s.spawn(VoidSender()).should == true;
...
@("cleanup.waitingsender.single") @safe
unittest {
	auto s = asyncScope();
...
@("cleanup.waitingsender.triple") @safe
unittest {
	auto s = asyncScope();
	s.spawn(waitingTask).should == true;
	s.spawn(waitingTask).should == true;
...
unittest {
	...
	auto s = asyncScope();
	s.spawn(justFrom(() shared {
		s.spawn(VoidSender());
	})).should == true;
	s.cleanup.syncWait.assumeOk;
}

@("spawn.value.transform") @safe
unittest {
...
unittest {
	...
	{
		auto s = asyncScope();
		s.spawn(waitingTask().onTermination(() shared {
			p.atomicStore(true);
		}));
	}
...
unittest {
	...
	{
		S s = S(asyncScope);
		s.s.spawn(waitingTask().onTermination(() shared {
			p.atomicStore(true);
		}));
	}
...
@("cleanup.assert.then") @safe
unittest {
	import concurrency.thread : ThreadSender;
	import concurrency.operations : then;
	auto s = asyncScope();

	s.cleanup.then(() shared {
		assert(false, "Ohh no!");
	}).syncWait.shouldThrow!Throwable;
}
